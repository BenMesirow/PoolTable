...
public:
	...
	bool operator!= (const Point v) {
		...
		//}
		//return 0;
		return (IN_RANGE(p[0], v[0]) == false) || (IN_RANGE(p[1], v[1]) == false) || (IN_RANGE(p[2], v[2]) == false);
	};
...
// Adds two matrices together, returns resultant matrix
...
inline Matrix operator*(const Matrix& M, double d) {
	return scale_mat(Vector(d,d,d)) * M;
}
